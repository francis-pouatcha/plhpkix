...
public class CertificationRequestCertifyPostAction extends GenericAction {

	public static final String RESPOND_OUTCOME="respond";

	private ProcessingResults<List<X509CertificateHolder>> processingResults;
	
	private final BuilderChecker checker = new BuilderChecker(CertificationRequestCertifyPostAction.class);
	public CertificationRequestCertifyPostAction(
			ActionContext actionContext,
			ProcessingResults<List<X509CertificateHolder>> processingResults) {
		super(actionContext);
		checker.checkNull(actionContext,processingResults);
		
		addProcessor(RESPOND_OUTCOME, CertificationRequestResponseActionProcessor.class);
